...
class AccountMove(models.Model):
    ...
    def _onchange_invoice_date(self):
        if self.invoice_date:
            if not self.invoice_payment_term_id and (not self.invoice_date_due or self.invoice_date_due < self.invoice_date.date()):
    ...
    def create_xml_file(self, previous_hash=0):
        ...
        if self.invoice_date > fields.Datetime.now():
        ...
        ubl_2_1 += '''
            ...
            <cbc:IssueTime>''' + self.invoice_date.strftime('%H:%M:%SZ') + '''</cbc:IssueTime>
        ...
        if bt_3 != '388':  # BR-KSA-56
            ...
            <cac:BillingReference>
                <cac:InvoiceDocumentReference>
                    ...
                    <cbc:IssueDate>''' + str(bt_25.invoice_date.strftime('%Y-%m-%d')) + '''</cbc:IssueDate>
                </cac:InvoiceDocumentReference>
            </cac:BillingReference>'''
        ubl_2_1 += '''
    ...
    def compliance_invoices_api(self):
        ...
        try:
            ...
            return {
                ...
                'view_mode': 'form',
                'res_id': self.id,
                'views': [(self.env.ref('invoice_zatca_integration.zatca_response').id, 'form')],
            }
    ...
    def invoices_clearance_single_api(self):
        ...
        try:
            ...
            if req.status_code == 500:
                ...
                file_name_specification = str(self.company_id.vat) + "_" + self.invoice_date.strftime('%Y%m%d') + "T" + self.invoice_date.strftime('%H%M%SZ') + "_" + str(self.id)
